...
StaticServer.prototype._buildIndexHtml = function () {
  ...
  return [
    ...
    '#dbg{position:fixed;top:4px;left:4px;right:4px;z-index:9999;',
    '  background:rgba(0,0,0,0.75);color:#0f0;font:11px monospace;',
    '  padding:4px 8px;border-radius:4px;pointer-events:none;white-space:nowrap;overflow:hidden;}',
    '</style>',
    '</head><body>',
    ...
    '      if(n.nodeType===3) btn.removeChild(n);', // nodeType 3 = TEXT_NODE
    '    });',
    '  }',
    ...
    '      if(!img) return;',
    '      purgeTextNodes(btn);',  // remove any stray Unicode text node
...
StaticServer.prototype._buildUploadPageHtml = function () {
  // All dynamic content is set via textContent/DOM methods in the client JS — no innerHTML.
  return [
    '<!DOCTYPE html>',
    '<html lang="en">',
    '<head>',
...
StaticServer.prototype._handleDelete = function (req, res, parsed) {
  ...
  if (!tapeId) {
    res.writeHead(400, json); res.end(JSON.stringify({ ok: false, message: 'Missing tape name' })); return;
  }
  ...
  if (path.dirname(tapeDir) !== this.tapesDir) {
    res.writeHead(403, json); res.end(JSON.stringify({ ok: false, message: 'Forbidden' })); return;
  }
  if (!fs.existsSync(tapeDir)) {
    res.writeHead(404, json); res.end(JSON.stringify({ ok: false, message: 'Tape not found' })); return;
  }
...
// ---------------------------------------------------------------------------
// Album art proxy (allows canvas to load Volumio art cross-origin)
// ---------------------------------------------------------------------------
